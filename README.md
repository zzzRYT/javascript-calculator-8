...
- [x] App Class
  ...
  - [x] custom구분자 추가하기
  - [x] 잘못된 입력이 들어오면 에러를 발생시킨다
  - [x] custom 구분자에 여러개가 들어오면 에러를 발생시킨다

- [x] Calculator Class

  - [x] calculator에 대한 validation 구현 (음수, 0 값에 대한 검증)
  - [x] Number값이 아닌 값 판별하기
  - [x] 실수 값 계산 (소수점 1번 째 자리 이하 반올림)
  - [x] 정수 범위를 넘어가는 수가 계산되면 에러 발생
  - [x] .이 구분자로 들어온다면, 정수로 계산
...
- [ ] [js 스타일 가이드](https://github.com/woowacourse/woowacourse-docs/tree/main/styleguide/javascript) 참고해서 작성했는지 확인
- [ ] node 버전 `22.19.0`확인
- [ ] 제출기한 확인
